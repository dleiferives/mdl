...
impl Executor {
            ...
            DataSub::Modify { storage, path, mode, source } => {
                if let Some(sel) = storage.strip_prefix("entity:") {
                    ...
                    let result = match mode.as_str() {
                        "set" => value().and_then(|value| {
                            self.world.entities.nbt_set(eid, path, value)
                        }),
                        "append" => value().and_then(|value| {
                            self.world.entities.nbt_append(eid, path, value)
                        }),
                        "prepend" => value().and_then(|value| {
                            self.world.entities.nbt_prepend(eid, path, value)
                        }),
                        "merge" => value().and_then(|value| {
                            self.world.entities.nbt_merge(eid, path, value)
                        }),
    ...
    fn data_source_value(&self, source: &str) -> Result<NbtValue, String> {
        ...
        Err(format!("expected `from storage` or `from entity`: {source}"))
    }
        ...
        if cmd.is_tag {
            if let Some(entries) = self.function_tags.get(&cmd.name).cloned() {
                ...
                for entry in entries {
                    ...
                    let entry_outcome =
                        self.execute_function_body(&entry, &mut sub_budget, None);

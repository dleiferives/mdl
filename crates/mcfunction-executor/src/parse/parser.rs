...
fn parse_data_get(words: &[String]) -> ParsedCommand {
    ...
    } else if words[1] == "block" && words.len() >= 5 {
        ParsedCommand::Data(DataCmd {
            subcommand: DataSub::Get {
                storage: format!("block:{} {} {}", words[2], words[3], words[4]),
                path: words.get(5).cloned().unwrap_or_default(),
                scale: words.get(6).and_then(|s| s.parse().ok()),
            },
        })
    } else {
        ParsedCommand::Data(DataCmd {
...
fn parse_data_remove(words: &[String]) -> ParsedCommand {
    ...
    let storage = if target_kind == "block" && words.len() >= 5 {
        format!("{storage_prefix}{} {} {}", words[2], words[3], words[4])
    } else {
        format!("{storage_prefix}{}", words[2])
    };
    let path = if target_kind == "block" {
        words.get(5).cloned().unwrap_or_default()
    } else {
...
fn parse_execute_condition(rest: &str) -> Option<(ExecuteCondition, String)> {
    ...
    match kind {
        ...
        "data" => {
            ...
            let (target, after_target) = if data_kind == "block" {
                let words: Vec<&str> = after_kind.split_whitespace().collect();
                if words.len() < 3 {
                    return None;
                }
                let target = words[..3].join(" ");
                let remaining = words[3..].join(" ");
                (target, remaining)
            } else {
                let (target, remaining) = split_first_word(after_kind);
                (target.to_owned(), remaining.to_owned())
            };
...
fn parse_function(rest: &str) -> ParsedCommand {
    let rest = rest.trim();
    let (raw_name, arguments) = split_first_word(rest);
    let is_tag = raw_name.starts_with('#');
    ...
    ParsedCommand::Function(FunctionCmd {
        name,
        is_tag,
        with_storage,
        inline_args,

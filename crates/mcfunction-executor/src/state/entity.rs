...
impl Entity {
    ...
    pub fn gamemode_str(&self) -> &str {
        self.gamemode.as_deref().unwrap_or("survival")
    }
}
...
struct SelectorFilters {
    ...
    gamemode_negated: bool,
    nbt_filter: Option<NbtValue>,
    scores_filter: Vec<(String, String)>,
}
...
impl EntityEngine {
    ...
    pub fn grant_advancement(&mut self, eid: u64, adv: &str) {
        if let Some(e) = self.entities.iter_mut().find(|e| e.id == eid) {
            if !e.advancements.contains(&adv.to_owned()) {
                e.advancements.push(adv.to_owned());
            }
        }
    }
    pub fn revoke_advancement(&mut self, eid: u64, adv: &str) {
        if let Some(e) = self.entities.iter_mut().find(|e| e.id == eid) {
            e.advancements.retain(|a| a != adv);
        }
    }
    pub fn has_advancement(&self, eid: u64, adv: &str) -> bool {
        self.entities
            .iter()
            .find(|e| e.id == eid)
            .map_or(false, |e| e.advancements.contains(&adv.to_owned()))
    }
    pub fn add_tag(&mut self, eid: u64, tag: &str) {
        if let Some(e) = self.entities.iter_mut().find(|e| e.id == eid) {
            if !e.tags.contains(&tag.to_owned()) {
                e.tags.push(tag.to_owned());
            }
        }
    }
    ...
    pub fn resolve_selector(&self, sel: &str, sb: &ScoreboardEngine) -> Vec<u64> {
        self.resolve_selector_at(sel, 0.0, 0.0, 0.0, sb)
    }
    pub fn resolve_selector_at(
        ...
        let (is_n, br) = if let Some(r) = sel.strip_prefix("@n") {
            (true, r)
        } else if let Some(r) = sel.strip_prefix("@e") {
            (false, r)
        } else {
            return vec![];
        };
        ...
        let sort = if is_n && f.sort == SortOrder::Arbitrary {
            SortOrder::Nearest
        } else {
            f.sort
        };
        let limit = if is_n {
            ...
            f.limit.unwrap_or(usize::MAX)
        };
        let rrx = f.ref_x.unwrap_or(rx);
        let rry = f.ref_y.unwrap_or(ry);
        let rrz = f.ref_z.unwrap_or(rz);

        let mut matches: Vec<(u64, f64)> = self
            .entities
            .iter()
    ...
    pub fn count(&self, sel: &str, sb: &ScoreboardEngine) -> usize {
        self.resolve_selector(sel, sb).len()
    }
    ...
    pub fn nbt_get(&self, eid: u64, path: &str) -> Option<NbtValue> {
        ...
        if path.is_empty() {
            return Some(e.nbt.clone());
        }
        let ops = NbtValue::parse_path(path).ok()?;
        if let Some(NbtPathOp::Key(key)) = ops.first() {
            match key.as_str() {
                "Pos" => {
                    let p = NbtValue::List(vec![
                        NbtValue::Double(e.x),
                        NbtValue::Double(e.y),
                        NbtValue::Double(e.z),
                    ]);
                    return p.resolve(&ops[1..]).cloned();
                }
                "Rotation" => {
                    let r = NbtValue::List(vec![NbtValue::Float(e.yaw), NbtValue::Float(e.pitch)]);
                    return r.resolve(&ops[1..]).cloned();
                }
                "Tags" => {
                    let t = NbtValue::List(
                        e.tags.iter().map(|t| NbtValue::String(t.clone())).collect(),
                    );
                    return t.resolve(&ops[1..]).cloned();
                }
    ...
    pub fn nbt_exists(&self, eid: u64, path: &str) -> bool {
        self.nbt_get(eid, path).is_some()
    }
    pub fn nbt_set(&mut self, eid: u64, path: &str, value: NbtValue) -> Result<(), String> {
        ...
        if ops.len() >= 1 {
            sync_entity_from_nbt_write(e, &ops, &value);
        }
    ...
    pub fn nbt_merge(&mut self, eid: u64, path: &str, source: NbtValue) -> Result<(), String> {
        let e = self
            ...
            .ok_or_else(|| format!("entity {eid} not found"))?;
        if path.is_empty() {
            let NbtValue::Compound(inc) = source else {
                return Err("merge needs compound".into());
            };
            let NbtValue::Compound(es) = &mut e.nbt else {
                return Err("not compound".into());
            };
            for (k, v) in inc {
                if let Some(p) = es.iter().position(|(key, _)| key == &k) {
                    es[p].1 = v;
                } else {
                    es.push((k, v));
                }
...
fn parse_bracket_filters(bracket: &str) -> SelectorFilters {
    ...
    for part in bracket.split(',') {
        ...
        match key {
            "type" => f.type_filter = Some(val.to_owned()),
            "tag" => f.tag_filter = Some(val.to_owned()),
            "limit" => f.limit = val.parse().ok(),
            "distance" => {
                if let Some(d) = val.strip_prefix("..") {
                    if let Some(dot) = d.find("..") {
                        f.distance_min = d[..dot].parse().ok();
                        f.distance_max = d[dot + 2..].parse().ok();
                    } else {
                        f.distance_max = d.parse().ok();
                    }
                } else if let Some(d) = val.strip_suffix("..") {
                    f.distance_min = d.parse().ok();
                } else if let Some(dot) = val.find("..") {
                    f.distance_min = val[..dot].parse().ok();
                    f.distance_max = val[dot + 2..].parse().ok();
                } else {
                    if let Ok(d) = val.parse() {
                        f.distance_min = Some(d);
                        f.distance_max = Some(d);
                    }
                }
            }
            "x" => f.ref_x = val.parse().ok(),
            "y" => f.ref_y = val.parse().ok(),
            "z" => f.ref_z = val.parse().ok(),
            "dx" => f.dx = val.parse().ok(),
            "dy" => f.dy = val.parse().ok(),
            "dz" => f.dz = val.parse().ok(),
            "x_rotation" => {
                let (a, b) = parse_float_range(val);
                f.pitch_min = a;
                f.pitch_max = b;
            }
            "y_rotation" => {
            ...
            "sort" => {
                f.sort = match val {
                    ...
                    _ => SortOrder::Arbitrary,
                }
            }
            "gamemode" => {
                if let Some(v) = val.strip_prefix('!') {
                    f.gamemode = Some(v.to_owned());
                    f.gamemode_negated = true;
                } else {
                    f.gamemode = Some(val.to_owned());
                }
            }
...
fn parse_float_range(val: &str) -> (Option<f32>, Option<f32>) {
    if let Some(r) = val.strip_prefix("..") {
        (None, r.parse().ok())
    } else if let Some(r) = val.strip_suffix("..") {
        (r.parse().ok(), None)
    } else if let Some(dot) = val.find("..") {
        (val[..dot].parse().ok(), val[dot + 2..].parse().ok())
    } else {
        let v = val.parse().ok();
        (v, v)
    }
...
fn parse_scores_filter(val: &str) -> Vec<(String, String)> {
    ...
    inner
        ...
        .map(|p| {
            let p = p.trim();
            let eq = find_eq_sep(p).unwrap_or(p.len());
            (p[..eq].trim().to_owned(), p[eq + 1..].trim().to_owned())
        })
        .collect()
}
...
fn entity_matches_filters(
    ...
    if let Some(ref t) = f.type_filter {
        let ts = e
            .entity_type
            .strip_prefix("minecraft:")
            .unwrap_or(&e.entity_type);
        if ts != t.as_str() && e.entity_type != *t && !(e.is_player() && t == "player") {
            return false;
        }
    }
    if let Some(ref t) = f.tag_filter {
        if !e.tags.contains(t) {
            return false;
        }
    }
    if let Some(ref m) = f.gamemode {
        if (e.gamemode_str() == m.as_str()) == f.gamemode_negated {
            return false;
        }
    }
    if f.distance_min.is_some() || f.distance_max.is_some() {
        let dx = e.x - rx;
        let dy = e.y - ry;
        let dz = e.z - rz;
        let dist = (dx * dx + dy * dy + dz * dz).sqrt();
        if let Some(min) = f.distance_min {
            if dist < min {
                return false;
            }
        }
        if let Some(max) = f.distance_max {
            if dist > max {
                return false;
            }
        }
    }
    if f.dx.is_some() || f.dy.is_some() || f.dz.is_some() {
        if let Some(dx) = f.dx {
            if e.x < rx || e.x > rx + dx {
                return false;
            }
    ...
    if let Some(min) = f.yaw_min {
        if e.yaw < min {
            return false;
        }
    }
    if let Some(max) = f.yaw_max {
        if e.yaw > max {
            return false;
        }
    }
    if let Some(min) = f.pitch_min {
        if e.pitch < min {
            return false;
        }
    }
    if let Some(max) = f.pitch_max {
        if e.pitch > max {
            return false;
        }
    }
    if let Some(ref nbt) = f.nbt_filter {
        if !nbt_compound_subset(&e.nbt, nbt) {
            return false;
        }
    }
    for (obj, range) in &f.scores_filter {
        let value = sb
            .get(&format!("entity_{}", e.id), obj)
            .or_else(|| sb.get(&e.id.to_string(), obj));
        let Some(value) = value else {
            return false;
        };
        if !eval_score_range(value, range) {
            return false;
        }
    }
    true
...
fn eval_score_range(v: i32, range: &str) -> bool {
    ...
    if let Some(r) = range.strip_suffix("..") {
        return v >= r.parse().unwrap_or(i32::MIN);
    }
    if let Some(r) = range.strip_prefix("..") {
        return v <= r.parse().unwrap_or(i32::MAX);
    }
    if let Some(dot) = range.find("..") {
        let min = range[..dot].parse().unwrap_or(i32::MIN);
        let max = range[dot + 2..].parse().unwrap_or(i32::MAX);
        return v >= min && v <= max;
    }
    false
}
...
fn sync_entity_from_nbt_write(e: &mut Entity, ops: &[NbtPathOp], val: &NbtValue) {
    if let NbtPathOp::Key(key) = &ops[0] {
        match key.as_str() {
            "Pos" => {
                if let NbtValue::List(vs) = val {
                    if ops.len() == 1 && vs.len() == 3 {
                        e.x = vs[0].as_f64();
                        e.y = vs[1].as_f64();
                        e.z = vs[2].as_f64();
                    }
    ...
    if ops.len() >= 2 {
        if let (NbtPathOp::Key(key), NbtPathOp::Index(idx)) = (&ops[0], &ops[1]) {
            match (key.as_str(), *idx) {
                ("Pos", 0) => e.x = val.as_f64(),
                ("Pos", 1) => e.y = val.as_f64(),
                ("Pos", 2) => e.z = val.as_f64(),
                ("Rotation", 0) => e.yaw = val.as_f32(),
                ("Rotation", 1) => e.pitch = val.as_f32(),
                _ => {}
            }
        }
    }

...
impl NbtValue {
    ...
    pub fn from_snbt(snbt: &str) -> Result<Self, String> {
        ...
        if let Some(r) = s.strip_suffix('b') { return Ok(Self::Byte(r.parse().map_err(|_| format!("bad byte: {r}"))?)); }
        if let Some(r) = s.strip_suffix('s') { return Ok(Self::Short(r.parse().map_err(|_| format!("bad short: {r}"))?)); }
        if let Some(r) = s.strip_suffix('L') { return Ok(Self::Long(r.parse().map_err(|_| format!("bad long: {r}"))?)); }
        if let Some(r) = s.strip_suffix('f') { return Ok(Self::Float(r.parse().map_err(|_| format!("bad float: {r}"))?)); }
        if let Some(r) = s.strip_suffix('d') { return Ok(Self::Double(r.parse().map_err(|_| format!("bad double: {r}"))?)); }

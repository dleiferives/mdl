...
impl NbtValue {
    ...
    pub fn from_scaled_i32(raw: i32, numeric_type: &str, scale: f64) -> Result<Self, String> {
        let scaled = f64::from(raw) * scale;
        match numeric_type {
            "byte" => Ok(Self::Byte(scaled as i8)),
            "short" => Ok(Self::Short(scaled as i16)),
            "int" => Ok(Self::Int(scaled as i32)),
            "long" => Ok(Self::Long(scaled as i64)),
            "float" => Ok(Self::Float(scaled as f32)),
            "double" => Ok(Self::Double(scaled)),
            _ => Err(format!("unknown numeric type: {numeric_type}")),
        }
    }
    ...
    pub fn to_snbt(&self) -> String {
        match self {
            ...
            Self::List(vals) => format!(
                "[{}]",
                vals.iter().map(Self::to_snbt).collect::<Vec<_>>().join(",")
            ),
    ...
    pub fn from_snbt(snbt: &str) -> Result<Self, String> {
        ...
        if let Some(r) = s.strip_suffix('b').or_else(|| s.strip_suffix('B')) {
            return Ok(Self::Byte(r.parse().map_err(|_| format!("bad byte: {r}"))?));
        }
        if let Some(r) = s.strip_suffix('s').or_else(|| s.strip_suffix('S')) {
            return Ok(Self::Short(
                r.parse().map_err(|_| format!("bad short: {r}"))?,
            ));
        }
        if let Some(r) = s.strip_suffix('l').or_else(|| s.strip_suffix('L')) {
            return Ok(Self::Long(r.parse().map_err(|_| format!("bad long: {r}"))?));
        }
        if let Some(r) = s.strip_suffix('f').or_else(|| s.strip_suffix('F')) {
            return Ok(Self::Float(
                r.parse().map_err(|_| format!("bad float: {r}"))?,
            ));
        }
        if let Some(r) = s.strip_suffix('d').or_else(|| s.strip_suffix('D')) {
            return Ok(Self::Double(
                r.parse().map_err(|_| format!("bad double: {r}"))?,
            ));
        }
        ...
        while pos < chars.len() {
            match chars[pos] {
                '[' => {
                    ...
                    if pos >= chars.len() {
                        return Err("unterminated `[` in NBT path".into());
                    }
                    let index: String = chars[start..pos].iter().collect();
                    pos += 1;
                    if index.is_empty() {
                        ops.push(NbtPathOp::AllElements);
                    } else {
                        ops.push(NbtPathOp::Index(
                            index
                                .parse()
                                .map_err(|_| format!("invalid NBT list index: {index}"))?,
                        ));
                    }
                }
                '.' => {
                ...
                '"' => {
                    let (key, next) = parse_quoted_path_key(&chars, pos)?;
                ...
                _ => {
                    ...
                    if key.is_empty() {
                        return Err("empty key in NBT path".into());
                    }
                    ops.push(NbtPathOp::Key(key));
                }
            }
    ...
    pub fn set_at(&mut self, ops: &[NbtPathOp], value: NbtValue) -> Result<(), String> {
        set_inner(self, ops, 0, value)
    }
    pub fn remove_at(&mut self, ops: &[NbtPathOp]) -> Result<(), String> {
        remove_inner(self, ops, 0)
    }
    pub fn merge_at(&mut self, ops: &[NbtPathOp], source: NbtValue) -> Result<(), String> {
        let NbtValue::Compound(incoming) = source else {
            return Err("merge needs compound".into());
        };
        let compound: &mut Vec<(String, NbtValue)> =
            resolve_mut_compound(self, ops).ok_or("resolve failed".to_owned())?;
        for (k, v) in incoming {
            if let Some(p) = compound.iter().position(|(key, _)| key == &k) {
                compound[p].1 = v;
            } else {
                compound.push((k, v));
            }
    ...
    pub fn append_at(&mut self, ops: &[NbtPathOp], value: NbtValue) -> Result<(), String> {
        let list: &mut Vec<NbtValue> =
            resolve_mut_list(self, ops).ok_or("resolve failed".to_owned())?;
        list.push(value);
        Ok(())
    }
...
fn step<'a>(cur: &'a NbtValue, op: &NbtPathOp) -> Option<&'a NbtValue> {
    match op {
        NbtPathOp::Key(k) => {
            let NbtValue::Compound(e) = cur else {
                return None;
            };
            e.iter().find(|(key, _)| key == k).map(|(_, v)| v)
        }
        NbtPathOp::Index(i) => {
...
fn set_inner(
    ...
    match &ops[index] {
        ...
        NbtPathOp::Index(i) => {
            let NbtValue::List(vals) = target else {
                return Err("cannot index non-list".into());
            };
        ...
        NbtPathOp::AllElements => {
            let NbtValue::List(vals) = target else {
                return Err("[] on non-list".into());
            };
            for v in vals {
                set_inner(v, ops, index + 1, value.clone())?;
            }
            Ok(())
        }
    }
...
fn insert_key(
    e: &mut Vec<(String, NbtValue)>,
    key: &str,
    next_op: Option<&NbtPathOp>,
    next_ops: &[NbtPathOp],
    value: NbtValue,
) -> Result<(), String> {
    if let Some(p) = e.iter().position(|(k, _)| k == key) {
        return set_inner(&mut e[p].1, next_ops, 0, value);
    }
    let mid = match next_op {
        Some(NbtPathOp::Index(_) | NbtPathOp::AllElements) => NbtValue::List(vec![]),
        _ => NbtValue::compound(vec![]),
    };
    e.push((key.to_owned(), mid));
    set_inner(&mut e.last_mut().unwrap().1, next_ops, 0, value)?;
...
fn remove_inner(target: &mut NbtValue, ops: &[NbtPathOp], index: usize) -> Result<(), String> {
    if index >= ops.len() - 1 {
        match &ops[index] {
            NbtPathOp::Key(k) => {
                let NbtValue::Compound(e) = target else {
            ...
            NbtPathOp::Index(i) => {
                let NbtValue::List(v) = target else {
                    return Err("not list".into());
                };
                let len = v.len() as i32;
                let idx = if *i < 0 { len + i } else { *i };
                if idx < 0 || idx >= len {
        ...
        match &ops[index] {
            NbtPathOp::Key(k) => {
                ...
                let p = e
                    ...
                    .position(|(key, _)| key == k)
                    .ok_or_else(|| format!("key not found: {k}"))?;
                remove_inner(&mut e[p].1, ops, index + 1)
            }
            NbtPathOp::Index(i) => {
                let NbtValue::List(v) = target else {
                    return Err("not list".into());
                ...
                remove_inner(&mut v[idx as usize], ops, index + 1)
            }
            NbtPathOp::AllElements => {
                let NbtValue::List(v) = target else {
                    return Err("not list".into());
...
fn resolve_mut_list<'a>(
    target: &'a mut NbtValue,
    ops: &[NbtPathOp],
) -> Option<&'a mut Vec<NbtValue>> {
    ...
    for op in ops {
        cur = match op {
            NbtPathOp::Key(k) => {
                ...
                let p = e.iter().position(|(key, _)| key == k)?;
                &mut e[p].1
            }
            NbtPathOp::Index(i) => {
                let NbtValue::List(v) = cur else { return None };
            ...
            NbtPathOp::AllElements => match cur {
                NbtValue::List(v) => return Some(v),
                _ => return None,
            },
        };
    }
...
fn resolve_mut_compound<'a>(
    target: &'a mut NbtValue,
    ops: &[NbtPathOp],
) -> Option<&'a mut Vec<(String, NbtValue)>> {
    let mut cur: &mut NbtValue = target;
    for op in ops {
        cur = match op {
            NbtPathOp::Key(k) => {
...
fn parse_list(s: &str) -> Result<NbtValue, String> {
    ...
    for e in els {
        vals.push(NbtValue::from_snbt(&e)?);
    }
...
fn parse_compound(s: &str) -> Result<NbtValue, String> {
    ...
    let inner = s
        .strip_prefix('{')
        .and_then(|r| r.strip_suffix('}'))
        .ok_or_else(|| format!("bad compound: {s}"))?;
    if inner.is_empty() {
        return Ok(NbtValue::compound(vec![]));
    }
    let pairs = split_snbt(inner);
    let mut entries = Vec::new();
    for pair in pairs {
        let colon = find_top_level_colon(&pair).ok_or_else(|| format!("missing colon: {pair}"))?;
...
fn parse_quoted_path_key(chars: &[char], start: usize) -> Result<(String, usize), String> {
    ...
    Err("unterminated quoted key in NBT path".into())
}

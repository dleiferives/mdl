...
fn sandbox(name: &str) -> PathBuf {
    ...
    fs::write(
        dp.join("pack.mcmeta"),
        r#"{"pack":{"description":"behavior","min_format":[107,1],"max_format":[107,1]}}"#,
    )
    .unwrap();
...
fn kill_removes_matching_entity() {
    ...
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"killme\"]}")
        .unwrap();
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"keep\"]}")
        .unwrap();

    exec.command("kill @e[tag=killme,limit=1]").unwrap();
    exec.command("scoreboard players set #alive v 1").unwrap();
    exec.command("execute unless entity @e[tag=killme] run scoreboard players set #alive v 2")
        .unwrap();
    exec.command("execute if entity @e[tag=keep] run scoreboard players add #alive v 1")
        .unwrap();
...
fn kill_removes_all_matching() {
    ...
    for i in 0..3 {
        exec.command(&format!(
            "summon minecraft:marker 0 0 0 {{Tags:[\"all{0}\"]}}",
            i
        ))
        .unwrap();
    }

    exec.command("execute if entity @e[tag=all0] run scoreboard players set #before v 1")
        .unwrap();
    ...
    exec.command("execute unless entity @e[tag=all0] run scoreboard players set #gone0 v 1")
        .unwrap();
...
fn tag_add_makes_entity_matchable() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"base\"]}")
        .unwrap();

    exec.command("tag @e[tag=base,limit=1] add dynamic")
        .unwrap();
    exec.command("execute if entity @e[tag=dynamic] run scoreboard players set #found v 1")
        .unwrap();
    ...
    exec.command("execute if entity @e[tag=base] run scoreboard players add #found v 1")
        .unwrap();
    exec.command("scoreboard players get #found v").unwrap();
...
fn tag_remove_stops_matching() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"temp\"]}")
        .unwrap();

    exec.command("execute if entity @e[tag=temp] run scoreboard players set #before v 1")
        .unwrap();
    exec.command("scoreboard players get #before v").unwrap();
    exec.wait_for_command_log("#before has 1").unwrap();

    exec.command("tag @e[tag=temp,limit=1] remove temp")
        .unwrap();

    exec.command("execute unless entity @e[tag=temp] run scoreboard players set #after v 1")
        .unwrap();
...
fn tag_list_returns_tags() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"a\",\"b\"]}")
        .unwrap();
    ...
    exec.command("execute if entity @e[tag=a] run scoreboard players set #still v 1")
        .unwrap();
...
fn tellraw_logs_chat_message() {
    ...
    exec.wait_for_command_log("[CHAT] test_message_12345")
        .unwrap();
    let _ = fs::remove_dir_all(&dir);
}
...
fn tp_absolute_position() {
    ...
    exec.command("execute as @e[type=armor_stand] at @s run teleport @s 100 64 100")
        .unwrap();
    exec.command(
...
fn tp_to_another_entity() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:marker 50 10 25 {Tags:[\"dest\"]}")
        .unwrap();
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"src\"]}")
        .unwrap();

    // tp src to dest
    exec.command("execute as @e[tag=src,limit=1] at @s run tp @s @e[tag=dest,limit=1]")
        .unwrap();
    let source_id = exec
        .executor()
        .world
        .entities
        .resolve_selector("@e[tag=src,limit=1]", &exec.executor().world.scoreboard)[0];
    let source = exec.executor().world.entities.get(source_id).unwrap();
    assert_eq!((source.x, source.y, source.z), (50.0, 10.0, 25.0));
    exec.command("execute as @e[tag=src] store result score #x v run data get entity @s Pos[0]")
        .unwrap();
    exec.command("scoreboard players get #x v").unwrap();
...
fn at_n_returns_nearest_within_distance() {
    ...
    exec.command("summon minecraft:marker 1 0 0 {Tags:[\"near\"]}")
        .unwrap();
    exec.command("summon minecraft:marker 10 0 0 {Tags:[\"mid\"]}")
        .unwrap();
    exec.command("summon minecraft:marker 100 0 0 {Tags:[\"far\"]}")
        .unwrap();

    // @n[distance=..5] should match the entity at x=1 (only one within 5)
    exec.command("execute if entity @n[distance=..5] run scoreboard players set #near v 1")
        .unwrap();
    ...
    exec.command("execute if entity @n[distance=9..15] run scoreboard players set #mid v 1")
        .unwrap();
...
fn at_n_type_and_tag() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:armor_stand 10 0 0 {Tags:[\"test\"]}")
        .unwrap();
    exec.command("summon minecraft:marker 1 0 0 {Tags:[\"test\"]}")
        .unwrap(); // closer but wrong type

    exec.command(
        "execute if entity @n[type=armor_stand,tag=test] run scoreboard players set #found v 1",
    )
    .unwrap();
    exec.command("scoreboard players get #found v").unwrap();
    exec.wait_for_command_log("#found has 1").unwrap();
...
fn nbt_filter_exact_match() {
    ...
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"hasdata\"]}")
        .unwrap();
    exec.command("data merge entity @e[tag=hasdata,limit=1] {Age:0s,Custom:5}")
        .unwrap();

    exec.command("execute if entity @e[nbt={Age:0s}] run scoreboard players set #age v 1")
        .unwrap();
    exec.command("scoreboard players get #age v").unwrap();
    exec.wait_for_command_log("#age has 1").unwrap();

    exec.command("execute if entity @e[nbt={Custom:5}] run scoreboard players set #custom v 1")
        .unwrap();
    ...
    exec.command("execute if entity @e[nbt={Age:1s}] run scoreboard players set #wrong v 1")
        .unwrap();
...
fn nbt_filter_missing_key_no_match() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"clean\"]}")
        .unwrap();

    exec.command(
        "execute unless entity @e[nbt={Nonexistent:1}] run scoreboard players set #absent v 1",
    )
    .unwrap();
...
fn data_modify_entity_set() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"m\"]}")
        .unwrap();

    exec.command("data modify entity @e[tag=m,limit=1] data.x set value 42")
        .unwrap();
    exec.command("data get entity @e[tag=m,limit=1] data.x")
        .unwrap();
    exec.wait_for_command_log("42").unwrap();

    exec.command("data modify entity @e[tag=m,limit=1] data.s set value \"hello\"")
        .unwrap();
    exec.command("data get entity @e[tag=m,limit=1] data.s")
        .unwrap();
...
fn data_modify_entity_copy_from_entity() {
    ...
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"src\"]}")
        .unwrap();
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"dst\"]}")
        .unwrap();

    exec.command("data modify entity @e[tag=src,limit=1] data.value set value 99")
        .unwrap();
    exec.command("data modify entity @e[tag=dst,limit=1] data.copied set from entity @e[tag=src,limit=1] data.value").unwrap();

    exec.command("data get entity @e[tag=dst,limit=1] data.copied")
        .unwrap();
...
fn data_modify_entity_copy_from_storage() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"e\"]}")
        .unwrap();

    // Set storage compound with a nested value
    exec.command("data modify storage test:v test.val set value 77")
        .unwrap();
    // Copy from storage path into entity NBT
    exec.command(
        "data modify entity @e[tag=e,limit=1] data.from_storage set from storage test:v test.val",
    )
    .unwrap();

    exec.command("data get entity @e[tag=e,limit=1] data.from_storage")
        .unwrap();
...
fn data_merge_entity() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"mrg\"]}")
        .unwrap();

    exec.command("data merge entity @e[tag=mrg,limit=1] {a:1,b:2}")
        .unwrap();
    exec.command("data merge entity @e[tag=mrg,limit=1] {c:3}")
        .unwrap(); // add new key

    exec.command("data get entity @e[tag=mrg,limit=1] a")
        .unwrap();
    exec.wait_for_command_log("1").unwrap();
    exec.command("data get entity @e[tag=mrg,limit=1] b")
        .unwrap();
    exec.wait_for_command_log("2").unwrap();
    exec.command("data get entity @e[tag=mrg,limit=1] c")
        .unwrap();
...
fn data_remove_entity_key() {
    ...
    exec.command("data merge entity @e[tag=rm,limit=1] {a:1,b:2}")
        .unwrap();
    exec.command("data remove entity @e[tag=rm,limit=1] a")
        .unwrap();

    // a should be gone
    exec.command(
        "execute if data entity @e[tag=rm,limit=1] b run scoreboard players set #b_still v 1",
    )
    .unwrap();
    ...
    // a should not exist
    exec.command(
        "execute unless data entity @e[tag=rm,limit=1] a run scoreboard players set #a_gone v 1",
    )
    .unwrap();
...
fn execute_if_data_entity() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:marker 0 0 0 {Tags:[\"d\"]}")
        .unwrap();
    exec.command("data merge entity @e[tag=d,limit=1] {present:1}")
        .unwrap();

    exec.command(
        "execute as @e[tag=d] if data entity @s present run scoreboard players set #yes v 1",
    )
    .unwrap();
    ...
    exec.command(
        "execute as @e[tag=d] unless data entity @s absent run scoreboard players set #no v 1",
    )
    .unwrap();
...
fn rotate_changes_entity_rotation() {
    ...
    exec.command(
        "execute as @e[tag=r] store result score #yaw v run data get entity @s Rotation[0]",
    )
    .unwrap();
...
fn scoreboard_operation_multiply() {
    ...
    exec.command("scoreboard players operation #a v *= #b v")
        .unwrap();
...
fn entity_pos_is_list_of_doubles() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:marker 10 20 30 {Tags:[\"p\"]}")
        .unwrap();

    exec.command("data get entity @e[tag=p,limit=1] Pos")
        .unwrap();

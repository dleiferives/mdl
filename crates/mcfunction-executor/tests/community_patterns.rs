...
fn nbt_list_shift() {
    ...
    exec.command("data modify storage mdl:test xs set value [1,2,3]")
        .unwrap();
...
fn execute_score_comparison() {
    ...
    exec.command("execute if score #a cmp = #b cmp run scoreboard players set #match cmp 1")
        .unwrap();
...
fn execute_score_match_range() {
    ...
    exec.command("scoreboard players set #matched rt 0")
        .unwrap();
    exec.command("execute if score #val rt matches 0..1 run scoreboard players set #matched rt 1")
        .unwrap();
...
fn nbt_score_roundtrip() {
    ...
    exec.command(
        "execute store result storage mdl:test result int 1 run scoreboard players get #src rtt",
    )
    .unwrap();
    exec.command("execute store result score #dst rtt run data get storage mdl:test result")
        .unwrap();
...
fn store_storage_types() {
    ...
    exec.command("data get storage mdl:test stored_byte")
        .unwrap();
    let line = exec
        .wait_for_command_log("has the following contents")
        .unwrap();
    ...
    exec.command("data get storage mdl:test stored_long")
        .unwrap();
    let line = exec
        .wait_for_command_log("has the following contents")
        .unwrap();
    ...
    exec.command("data get storage mdl:test stored_double")
        .unwrap();
...
fn tape_operations() {
    ...
    exec.command("execute store result score #val var run data get storage mdl:state Tape[0]")
        .unwrap();
    ...
    exec.command(
        "execute store result storage mdl:state Tape[0] int 1 run scoreboard players get #val var",
    )
    .unwrap();
...
fn scheduling() {
    ...
    exec.executor_mut().advance_ticks(3).unwrap();

...
fn full_crafting_pipeline_with_config() {
    ...
    maybe_copy(
        &src.join("config/show/init.mcfunction"),
        &func.join("config/show/init.mcfunction"),
    );
    ...
    e.command("data modify storage test:tmp Items append from entity @e[tag=slot.0,limit=1] item")
        .unwrap();

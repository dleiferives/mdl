...
fn craft_oak_planks_to_crafting_table() {
    ...
    // Manual setup
    e.command("scoreboard objectives add dynamic_crafting.values dummy")
        .unwrap();
    ...
    e.command("execute in dynamic_crafting:crafters run data get block 0 0 0 Items[0].Slot")
        .unwrap();
    ...
    e.command("data get entity @e[tag=result,limit=1] Item.Count")
        .unwrap();
    e.wait_for_command_log("1b").unwrap();

    // Clean up storage
    e.command("data remove storage test:tmp Items").unwrap();
    e.command("execute unless data storage test:tmp Items run scoreboard players set #clean v 1")
        .unwrap();
...
fn run_load_function_and_config() {
    ...
    fs::write(
        dp.join("pack.mcmeta"),
        r#"{"pack":{"description":"min","min_format":[107,1],"max_format":[107,1]}}"#,
    )
    .unwrap();
    ...
    if src.join("config/reset.mcfunction").exists() {
        fs::copy(
            src.join("config/reset.mcfunction"),
            func.join("config/reset.mcfunction"),
        )
        .unwrap();
    }
    ...
    e.command("scoreboard players set #v dynamic_crafting.values 0")
        .unwrap();
    // Verify storage config was set by reset
    e.command("data get storage dynamic_crafting:config values.max_per_chunk")
        .unwrap();
    let line = e
        .wait_for_command_log("has the following contents")
        .unwrap();
    assert!(
        line.contains("16") || line.contains("max_per_chunk"),
        "got: {line}"
    );
...
fn remove_crafting_table_cleanup() {
    ...
    e.command("summon minecraft:item_display 0 0 0 {Tags:[\"slot.visual\"]}")
        .unwrap();
    e.command("summon minecraft:interaction 0 0 0 {Tags:[\"slot.hitbox\"]}")
        .unwrap();
    e.command("summon minecraft:interaction 0 0 0 {Tags:[\"result.hitbox\"]}")
        .unwrap();
    e.command("summon minecraft:text_display 0 0 0 {Tags:[\"slot.count\"]}")
        .unwrap();
    ...
    // Verify they exist
    e.command("execute if entity @e[tag=slot.visual] run scoreboard players set #before v 1")
        .unwrap();
    ...
    e.command(
        "data modify entity @e[tag=slot.visual,limit=1] item set value {id:\"minecraft:stone\"}",
    )
    .unwrap();
    e.command("loot spawn 0 65 0 loot dynamic_crafting:drop")
        .unwrap();
    ...
    // Verify all gone
    e.command("execute unless entity @e[tag=slot.visual] run scoreboard players add #clean v 1")
        .unwrap();
    e.command("execute unless entity @e[tag=result.hitbox] run scoreboard players add #clean v 1")
        .unwrap();
...
fn item_display_init_animation_merge() {
    ...
    e.command("data get entity @e[tag=init,limit=1] transformation")
        .unwrap();
    let line = e
    ...
    assert!(
        line.contains("0.275"),
        "transformation should have scale: {line}"
    );

    // Remove init tag (after animation completes)
    e.command("tag @e[tag=init] remove init").unwrap();
    e.command("execute unless entity @e[tag=init] run scoreboard players set #done v 1")
        .unwrap();

...
fn sandbox(name: &str) -> PathBuf {
    ...
    fs::write(
        dp.join("pack.mcmeta"),
        r#"{"pack":{"description":"spatial","min_format":[107,1],"max_format":[107,1]}}"#,
    )
    .unwrap();
...
fn summon_and_teleport_absolute() {
    ...
    exec.command("execute as @e[type=armor_stand] run teleport @s 10 20 30")
        .unwrap();
    exec.command(
        "execute as @e[type=armor_stand] store result score #x pos run data get entity @s Pos[0]",
    )
    .unwrap();
...
fn teleport_relative() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:armor_stand 10 0 10")
        .unwrap();
    exec.command("execute as @e[type=armor_stand] at @s run teleport @s ~ ~5 ~")
        .unwrap();
    exec.command(
        "execute as @e[type=armor_stand] store result score #y pos run data get entity @s Pos[1]",
    )
    .unwrap();
...
fn execute_positioned_relative() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:armor_stand 50 0 50")
        .unwrap();
    exec.command(
        "execute as @e[type=armor_stand] at @s positioned ~10 ~20 ~ run teleport @s ~ ~5 ~",
    )
    .unwrap();
    exec.command(
        "execute as @e[type=armor_stand] store result score #x pos run data get entity @s Pos[0]",
...
fn execute_anchored_and_align() {
    ...
    exec.wait_for_command_log("#x has 10").unwrap();
    exec.command(
        "execute as @e[type=armor_stand] store result score #z pos run data get entity @s Pos[2]",
    )
    .unwrap();
...
fn execute_in_dimension() {
    ...
    let mut exec = executor(&dir);
    exec.command("summon minecraft:armor_stand 0 0 0").unwrap();
    exec.command("execute in minecraft:the_nether run teleport @e[type=armor_stand] 3 5 3")
        .unwrap();
    let entity_id = exec.executor().world.entities.resolve_selector(
        "@e[type=armor_stand,limit=1]",
        &exec.executor().world.scoreboard,
    )[0];
    let entity = exec.executor().world.entities.get(entity_id).unwrap();
    assert_eq!(entity.dimension, "minecraft:the_nether");
    exec.command(
        "execute as @e[type=armor_stand] store result score #x pos run data get entity @s Pos[0]",
    )
    .unwrap();
...
fn selector_with_distance_filter() {
    ...
    exec.command("summon minecraft:armor_stand 100 0 0")
        .unwrap();
    exec.command("execute if entity @e[distance=..15] run scoreboard players set #found pos 1")
        .unwrap();
...
fn multiple_entities_as_forking() {
    ...
    exec.command("summon minecraft:armor_stand 10 0 0").unwrap();
    exec.command("execute as @e[type=armor_stand] at @s run teleport @s ~ ~10 ~")
        .unwrap();
    exec.command("execute if entity @e[y=10,distance=..1] run scoreboard players set #match pos 1")
        .unwrap();
...
fn setblock_and_if_block_condition() {
    ...
    exec.command("execute if block 5 64 5 minecraft:stone run scoreboard players set #found pos 1")
        .unwrap();
    exec.command("scoreboard players get #found pos").unwrap();
    exec.wait_for_command_log("#found has 1").unwrap();
    exec.command("scoreboard players set #found pos 0").unwrap();
    exec.command(
        "execute if block 5 64 5 minecraft:diamond_block run scoreboard players set #found pos 1",
    )
    .unwrap();
    exec.command("scoreboard players get #found pos").unwrap();
...
fn setblock_verify_entity_position() {
    ...
    exec.command("summon minecraft:armor_stand 0 0 0").unwrap();
    exec.command("teleport @e[type=armor_stand] 100 64 100")
        .unwrap();
    let entity_id = exec.executor().world.entities.resolve_selector(
        "@e[type=armor_stand,limit=1]",
        &exec.executor().world.scoreboard,
    ...
    let entity = exec.executor().world.entities.get(entity_id).unwrap();
    assert_eq!((entity.x, entity.y, entity.z), (100.0, 64.0, 100.0));
    exec.command(
        "execute if block 100 64 100 minecraft:stone run scoreboard players set #at_stone pos 1",
    )
    .unwrap();
    exec.command("scoreboard players get #at_stone pos")
        .unwrap();
    exec.wait_for_command_log("#at_stone has 1").unwrap();
    exec.command("scoreboard players set #at_stone pos 0")
        .unwrap();
    exec.command(
        "execute if block 100 64 101 minecraft:stone run scoreboard players set #at_stone pos 1",
    )
    .unwrap();
    exec.command("scoreboard players get #at_stone pos")
        .unwrap();

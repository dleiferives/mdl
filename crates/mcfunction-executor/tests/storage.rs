...
fn sandbox(name: &str) -> PathBuf {
    ...
    fs::write(
        dp.join("pack.mcmeta"),
        r#"{"pack":{"description":"storage","min_format":[107,1],"max_format":[107,1]}}"#,
    )
    .unwrap();
...
fn storage_set_and_get_compound() {
    ...
    exec.command("data modify storage mdl:test test_compound set value {a:1,b:2}")
        .unwrap();
    exec.command("data get storage mdl:test test_compound")
        .unwrap();
    let line = exec
        .wait_for_command_log("has the following contents")
        .unwrap();
    assert!(
        line.contains("{a:1,b:2}") || line.contains("{b:2,a:1}"),
        "got: {line}"
    );
...
fn storage_list_operations() {
    ...
    exec.command("data modify storage mdl:test ordered set value [1,2]")
        .unwrap();
    ...
    exec.command("data modify storage mdl:test ordered append value 3")
        .unwrap();
    exec.command("data get storage mdl:test ordered").unwrap();
    let line = exec
        .wait_for_command_log("has the following contents")
    ...
    exec.command("data modify storage mdl:test ordered prepend value 0")
        .unwrap();
    exec.command("data get storage mdl:test ordered").unwrap();
    let line = exec
        .wait_for_command_log("has the following contents")
...
fn storage_copy_and_merge() {
    ...
    exec.command("data modify storage mdl:test source set value {b:2,a:1}")
        .unwrap();
    exec.command("data modify storage mdl:test copied set from storage mdl:test source")
        .unwrap();
    ...
    exec.command("data modify storage mdl:test copied merge value {c:3}")
        .unwrap();
    exec.command("data get storage mdl:test copied").unwrap();
    let line = exec
        .wait_for_command_log("has the following contents")
...
fn storage_remove_key() {
    ...
    exec.command("data modify storage mdl:test test set value {a:1,b:2,c:3}")
        .unwrap();
...
fn execute_if_data_condition() {
    ...
    exec.command("scoreboard objectives add test dummy")
        .unwrap();
    exec.command("scoreboard players set #flag test 0").unwrap();

    // Condition false (doesn't exist yet)
    exec.command("execute if data storage mdl:test result run scoreboard players set #flag test 1")
        .unwrap();
    ...
    // Create it
    exec.command("data modify storage mdl:test result set value 1")
        .unwrap();
...
fn execute_store_result_score() {
    ...
    exec.command("execute store result score #dest calc run scoreboard players get #src calc")
        .unwrap();

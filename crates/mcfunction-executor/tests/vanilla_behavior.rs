...
fn sandbox(name: &str) -> PathBuf {
    ...
    fs::write(dp.join("pack.mcmeta"), r#"{"pack":{"description":"vb","min_format":[107,1],"max_format":[107,1]}}"#).unwrap();
...
fn write_fn(dir: &PathBuf, name: &str, content: &str) {
    fs::write(
        dir.join("world").join("datapacks").join("test").join("data/test/function").join(format!("{name}.mcfunction")),
        content,
    ).unwrap();
}
...
fn crafting_table_marker_tick_cooldown() {
    ...
    e.command("summon minecraft:marker 0 0 0 {Tags:[\"ct\"]}").unwrap();
    e.command("scoreboard players set @e[tag=ct,limit=1] dc 7").unwrap();
    e.command("scoreboard players remove @e[tag=ct,limit=1] dc 1").unwrap();
    e.command("scoreboard players get @e[tag=ct,limit=1] dc").unwrap();
...
fn slot_display_entity_item_nbt() {
    ...
    let mut e = exec(&d);
    e.command("summon minecraft:item_display 0 0 0 {Tags:[\"s0\"]}").unwrap();
    e.command("data modify entity @e[tag=s0,limit=1] item set value {id:\"minecraft:oak_planks\",Count:1b}").unwrap();
    e.command("data get entity @e[tag=s0,limit=1] item.id").unwrap();
    e.wait_for_command_log("oak_planks").unwrap();
    e.command("data get entity @e[tag=s0,limit=1] item.Count").unwrap();
...
fn if_data_entity_item_presence() {
    ...
    e.command("data modify entity @e[tag=s,limit=1] item set value {id:\"minecraft:stick\"}").unwrap();
    e.command("execute if data entity @e[tag=s,limit=1] item run scoreboard players set #ok v 1").unwrap();
    e.command("scoreboard players get #ok v").unwrap();
    e.wait_for_command_log("#ok has 1").unwrap();
    e.command("data remove entity @e[tag=s,limit=1] item").unwrap();
    e.command("execute unless data entity @e[tag=s,limit=1] item run scoreboard players set #gone v 1").unwrap();
...
fn slot_items_to_storage_to_block() {
    ...
    let mut e = exec(&d);
    for i in 0..3 {
        e.command(&format!("summon minecraft:item_display 0 0 0 {{Tags:[\"s\",\"s{i}\"]}}")).unwrap();
    }
    e.command("data modify entity @e[tag=s0,limit=1] item set value {id:\"minecraft:oak_planks\",Count:1b}").unwrap();
    e.command("data modify entity @e[tag=s1,limit=1] item set value {id:\"minecraft:stick\",Count:1b}").unwrap();

    e.command("data merge storage test:tmp {Items:[]}").unwrap();
    e.command("data modify storage test:tmp Items append from entity @e[tag=s0,limit=1] item").unwrap();
    e.command("data modify storage test:tmp Items append from entity @e[tag=s1,limit=1] item").unwrap();
...
fn result_item_spawn_and_copy() {
    ...
    let mut e = exec(&d);
    e.command("summon minecraft:item_display 0 0 0 {Tags:[\"vis\"]}").unwrap();
    ...
    e.command("data modify entity @e[tag=r,limit=1] Item set from entity @e[tag=vis,limit=1] item").unwrap();
    e.command("data modify entity @e[tag=r,limit=1] PickupDelay set value 4").unwrap();
    e.command("data get entity @e[tag=r,limit=1] Item.id").unwrap();
...
fn config_compound_predicate() {
    let d = sandbox("cfg");
    let mut e = exec(&d);
    e.command("data modify storage test:c v set value {tick_dist:32,limit:16,sound:1}").unwrap();
    e.command("execute if data storage test:c {v:{sound:1}} run scoreboard players set #ok v 1").unwrap();
    e.command("scoreboard players get #ok v").unwrap();
    e.wait_for_command_log("#ok has 1").unwrap();
    let _ = fs::remove_dir_all(&d);
}
...
fn score_range_all_forms() {
    ...
    e.command("execute if score #v x matches 50 run scoreboard players set #f x 1").unwrap();
    assert_eq!(get_score(&mut e, "#f", "x"), 1);
    e.command("execute if score #v x matches 20.. run scoreboard players set #f x 2").unwrap();
    assert_eq!(get_score(&mut e, "#f", "x"), 2);
    e.command("execute if score #v x matches ..100 run scoreboard players set #f x 3").unwrap();
    assert_eq!(get_score(&mut e, "#f", "x"), 3);
    e.command("execute if score #v x matches 40..60 run scoreboard players set #f x 4").unwrap();
...
fn score_compare_all_ops() {
    ...
    e.command("execute if score #a x = #b x run scoreboard players set #f x 1").unwrap();
    assert_eq!(get_score(&mut e, "#f", "x"), 1);
    e.command("scoreboard players set #a x 5").unwrap();
    e.command("execute if score #a x < #b x run scoreboard players set #f x 2").unwrap();
    assert_eq!(get_score(&mut e, "#f", "x"), 2);
    e.command("scoreboard players set #a x 15").unwrap();
    e.command("execute if score #a x > #b x run scoreboard players set #f x 3").unwrap();
    assert_eq!(get_score(&mut e, "#f", "x"), 3);
    e.command("scoreboard players set #a x 10").unwrap();
    e.command("execute if score #a x >= #b x run scoreboard players set #f x 4").unwrap();
    assert_eq!(get_score(&mut e, "#f", "x"), 4);
    let _ = fs::remove_dir_all(&d);
}
...
fn entity_kill_and_tag_lifecycle() {
    let d = sandbox("life");
    let mut e = exec(&d);
    for _ in 0..3 { e.command("summon minecraft:marker 0 0 0 {Tags:[\"a\"]}").unwrap(); }
    e.command("tag @e[tag=a] add b").unwrap();
    e.command("execute if entity @e[tag=b] run scoreboard players set #ok v 1").unwrap();
    e.command("scoreboard players get #ok v").unwrap();
    e.wait_for_command_log("#ok has 1").unwrap();
    e.command("tag @e[tag=a] remove b").unwrap();
    e.command("execute unless entity @e[tag=b] run scoreboard players set #rm v 1").unwrap();
    e.command("scoreboard players get #rm v").unwrap();
    e.wait_for_command_log("#rm has 1").unwrap();
    e.command("kill @e[tag=a]").unwrap();
    e.command("execute unless entity @e[tag=a] run scoreboard players set #dead v 1").unwrap();
...
fn crafters_dimension_blocks_dont_conflict() {
    ...
    e.command("data modify block 0 0 0 test set value 42").unwrap();
...
fn tp_relative_from_current() {
    ...
    e.command("execute as @e[type=armor_stand] at @s run teleport @s ~5 ~3 ~-2").unwrap();
    e.command("execute as @e[type=armor_stand] store result score #x v run data get entity @s Pos[0]").unwrap();
